...
class SamplingBinsBatcher(torch.utils.data.sampler.Sampler):
    ...
    Arguments:
        ...
        `peak_qvals_array`: an N x T x 2 array of q-values and peak ranks by
            q-values for peaks underlying bins, generated by
            `create_ENCODE_binary_bins.py`; only needed if `peak_retention` is
            used; specifies the -log(q) and rank (small rank is high -log(q))
            of each peak in each bin
    ...
    def __init__(
        ...
        peak_retention=None, peak_qvals_array=None, shuffle_before_epoch=False,
        shuffle_seed=None
    ):
        ...
        if peak_retention is not None:
            assert peak_qvals_array is not None, \
                "If peak retention is on, an array of q-values and peak ranks must be given"
           
            # Start keeping nothing
            keep_mask = np.zeros(len(pos_mask), dtype=bool)
            num_tasks = peak_qvals_array.shape[1]
            for i in range(num_tasks):
                ranks = peak_qvals_array[:, i, 1]
...
def create_data_loader(
    ...
    shuffle_seed, peak_qvals_npy_or_array=None, chrom_set=None, shuffle=True,
    return_coords=False
):
    """
    ...
    Arguments:
        ...
        `peak_qvals_npy_or_array`: either the path to an N x T array, or the
    ...
    if type(peak_qvals_npy_or_array) is str:
        peak_qvals_array = np.load(peak_qvals_npy_or_array)
    else:
        peak_qvals_array = peak_qvals_npy_or_array  # Could be None
    bins_batcher = SamplingBinsBatcher(
        bin_labels_array, batch_size, negative_ratio, chroms_keep=chrom_set,
        peak_retention=peak_retention, peak_qvals_array=peak_qvals_array,
...
def main():
    ...
    peak_qvals_array = np.load(peak_qvals_npy, allow_pickle=True)
    loader = create_data_loader(
        labels_hdf5, bin_labels_array, return_coords=True,
        chrom_set=train_chroms, peak_retention=0.01,
        peak_qvals_npy_or_array=peak_qvals_array, shuffle_seed=123
    )
