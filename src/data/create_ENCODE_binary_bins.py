...
def extract_bin_signals(
    labels_hdf5, label_sources_hdf5, bin_labels_array, labelgen_tasks_tsv,
    check_coords=True
):
    """
    From an HDF5 of peak sources generated by
    `generate_ENCODE_..._binary_labels.sh`, extracts the signal values stored
    for each bin/task to a NumPy array.
    Arguments:
    ...
    Returns an N x T x 2 array of signal values and peak ranks for each bin and
    task. That is, for each bin/task, this array records the signal strength of
    the underlying peak, and the rank of the peak by signal strength (rank of 1
    is highest signal). If a bin does not have an underlying peak, the signal
    values and peak rank will both be 0. Note that the N rows in this array are
    exactly parallel to the N rows in `labels_hdf5`.
    """
    ...
    # Allocate array for signals and ranks
    signal_ranks = np.zeros((num_bins, num_tasks, 2))  # Defaults to 0, 0
    peak_signal_ranks = np.zeros((num_pos, num_tasks, 2))  # Just for the peaks
    ...
    with open(labelgen_tasks_tsv, "r") as f:
        ...
        for line in f:
            ...
            # Drop duplicate peaks, keeping the one with the best signal
            peaks_table = peaks_table.sort_values("signal", ascending=False)
            peaks_table = peaks_table.drop_duplicates(
                subset=["chrom", "peak_start", "peak_end"],
                keep="first"
            )
            # Set ranks column
            peaks_table["rank"] = peaks_table["signal"].rank(
    ...
    for i in tqdm.trange(num_chunks):
        ...
        # Turn intervals into B x T x 2, and put start before end
    ...
    for i in range(num_tasks):
        ...
        task_signal_ranks = merged[["signal", "rank"]].values
        # Fill in the signals and ranks, turning anything unmatched into 0;
        # this is because some coordinates in the source table won't have a
        # match, as they are coordinates for another task)
        peak_signal_ranks[:, i] = np.nan_to_num(task_signal_ranks, nan=0)

    labels_h5_reader.close()
    sources_h5_reader.close()

    # For the bins with peak locations, set the signal values and ranks
    print("Setting signal values and ranks...")
    signal_ranks[pos_mask] = peak_signal_ranks
...
def main(
    ...
    out_labels_hdf5, bin_labels_npy, peak_signals_npy, skip_check_coords
):
    """
    ...
    3. Matches the underlying source peaks in each bin to the signal value from
        the original peaks BED files, noting the signal strength and rank of
        peak confidence for each task in each bin
    """
    ...
    if not peak_signals_npy:
        peak_signals_npy = os.path.join(
            processed_base, "%s_peak_signals.npy" % cell_line_or_tf
        )
    
    os.makedirs(os.path.dirname(out_labels_hdf5), exist_ok=True)
    os.makedirs(os.path.dirname(bin_labels_npy), exist_ok=True)
    os.makedirs(os.path.dirname(peak_signals_npy), exist_ok=True)
    ...
    # Create/save the peak signal values
    peak_signals_array = extract_bin_signals(
        labels_hdf5, label_sources_hdf5, bin_labels_array, labelgen_tasks_tsv,
        check_coords=(not skip_check_coords)
    )
    np.save(peak_signals_npy, peak_signals_array)
